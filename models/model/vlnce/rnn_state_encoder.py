...
def logical_not(tensor: torch.Tensor) -> torch.Tensor:
    ...
    return (~tensor.byte()).to(tensor.dtype)
...
def build_rnn_inputs(
    ...
    rnn_states = torch.where(
        not_dones.view(1, -1, 1).index_select(1, episode_starts).byte(),
        rnn_states,
...
class RNNStateEncoder(nn.Module):
    ...
    def single_forward(
        ...
        hidden_states = torch.where(
            masks.view(1, -1, 1).byte(), hidden_states, hidden_states.new_zeros(())
        )

        idx = 1 if self.batch_first else 0
        x, hidden_states = self.rnn(x.unsqueeze(idx), self.unpack_hidden(hidden_states))
    ...
    def seq_forward(self, x, hidden_states, masks) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # FIXME untested with batch_first=False
        # if self.batch_first:
        hidden_states_t = hidden_states.transpose(0, 1)
        x_seq = x.unsqueeze(1)

        x_seq, hidden_states = self.rnn(x_seq, hidden_states_t)
        x = x_seq.squeeze(1)

        # if self.batch_first:
        hidden_states = hidden_states_t.transpose(0, 1)
        # x, hidden_states = self.rnn(x_seq, self.unpack_hidden(hidden_states))
        # hidden_states = self.pack_hidden(hidden_states)
    ...
    def forward(self, x, hidden_states, masks) -> Tuple[torch.Tensor, torch.Tensor]:
        return hidden_states.squeeze(0), hidden_states
        # hidden_states = hidden_states.permute(1, 0, 2)
        # len_idx = 1 if self.batch_first else 0
        # if x.size(len_idx) == hidden_states.size(1):
        # if len(x.shape) == 2:
...
def build_rnn_state_encoder(
    ...
    if rnn_type == "gru":
        return GRUStateEncoder(input_size, hidden_size, num_layers, batch_first)
    elif rnn_type == "lstm":
        return LSTMStateEncoder(input_size, hidden_size, num_layers, batch_first)
    else:
